...
function showHitokoto() {
  ...
  fetch("https://v1.hitokoto.cn")
    ...
    .then((result) => {
      const text = `This saying comes from... <span>「${result.from}」</span>，是 <span>${result.creator}</span> 在 hitokoto.cn 投稿的。`;
...
const tools = {
  ...
  photo: {
    ...
    callback: () => {
      showMessage("Is the photo taken? Isn't it very cute?", 6000, 9);
  ...
  quit: {
    ...
    callback: () => {
      ...
      showMessage(
        "May you reunite with the important person someday.",
        2000,
        11
      );
