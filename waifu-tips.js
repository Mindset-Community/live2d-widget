...
!(function () {
    ...
    async loadModel(t, s, n) {
      if (
        ...
        loadlive2d("live2d", `${this.apiPath}get/?id=${t}-${s}`),
          console.log(`The Live2D model ${t}-${s} has finished loading.`);
    }
    async loadRandModel() {
      ...
      if (this.useCDN) {
        ...
        loadlive2d("live2d", `${this.cdnPath}model/${s}/index.json`),
          o("Are my new clothes nice?", 4e3, 10);
      } else
        fetch(`${this.apiPath}rand_textures/?id=${t}-${s}`)
          .then((e) => e.json())
          .then((e) => {
            1 !== e.textures.id || (1 !== s && 0 !== s)
              ? this.loadModel(t, e.textures.id, "Are my new clothes nice?")
  ...
  const n = {
    ...
    photo: {
      ...
      callback: () => {
        o("Did you take the photo? Isn't it very cute?", 6e3, 9),
    ...
    quit: {
      ...
      callback: () => {
        localStorage.setItem("waifu-display", Date.now()),
          o("May you reunite with the important person someday.", 2e3, 11),
            ...
            if ("" !== document.referrer) {
              ...
              return location.hostname === e.hostname
                ...
                : ((o = s in n ? n[s] : e.hostname),
                  `Hello! Friend from <span>${o}</span>.<br>${t}`);
            }
            return t;
          })(t.time),
          7e3,
